"""
Kansas Host–Parasite Atlas Version 2.2 processor.
...
def parse_year(v):
    """
    Return only an unambiguous four-digit year from VERBATIM_DATE.
    Two-digit years are intentionally left unknown rather than guessing
    the century (for example, 05 could mean 1905 or 2005).
    """
    if pd.isna(v) or not str(v).strip():
        return None
    match = re.search(r"(?<!\d)(18\d{2}|19\d{2}|20\d{2})(?!\d)", str(v))
...
def main(src, outdir):
    ...
    status_counter, part_counter = Counter(), Counter()
    years = []
    ...
    for _, r in df.iterrows():
        ...
        for p in safe_json(r.get("PARTDETAIL")):
            ...
            parsed = parse_counts(p.get("part_remark"))
            agg.update(parsed)

            info = {
                "part_name": name,
                "part_id": p.get("partID"),
                "remark": p.get("part_remark"),
                "preservation": preservation(p) or None,
                "condition": p.get("condition"),
                "count": p.get("part_count"),
                "parsed_counts": parsed,
            }
            pparts.append(info)
            part_counter[lname] += 1

            part_rows.append({
                "host_guid": r.get("GUID"),
                "host_species": r.get("SPECIES"),
                "host_genus": r.get("GENUS"),
                "part_name": name,
                "part_id": p.get("partID"),
                "part_remark": p.get("part_remark"),
                "preservation": preservation(p),
                "condition": p.get("condition"),
                "part_count": p.get("part_count"),
                **{f"parsed_{k}_count": parsed.get(k) for k in GROUP_PATTERNS},
            })

        if detected and not_detected:
            status = "mixed screening"
        elif detected:
            status = "detected"
        elif not_detected:
            status = "not detected"
        elif pparts:
            status = "material present"
        else:
            status = "not recorded"

        status_counter[status] += 1
        guid = str(r.get("GUID"))
        year = parse_year(r.get("VERBATIM_DATE"))
        if year is not None:
            years.append(year)
        ...
        host_rows.append({
            ...
            "not_detected": "; ".join(not_detected),
    ...
    (outdir / "specimens.geojson").write_text(
